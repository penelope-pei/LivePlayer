...
public class DBAdapter {
	...
	//数据表名,一个数据库可以创建多个表
	private static final String DATABASE_TABLE = "musiclist";
	//音乐播放列表
	private static final String DATABASE_HISTORY = "historylist";
	//数据库版本号
	private static final int DATABASE_VERSION = 1;
	
	//创建数据库SQL语句
	//总共有10列，分别为：歌曲名，歌手名，歌曲路径，歌曲时长，歌曲大小，歌曲专辑,是否喜爱，歌词路径，专辑路径
	private static final String DATABASE_CREATE =
			"create table musiclist (_id integer primary key autoincrement, "
	...
	public MusicInfo find(Integer id){  
		...
		SQLiteDatabase db = DBHelper.getReadableDatabase();
		//得到游标  
        Cursor cursor = db.rawQuery("select * from musiclist where _id=?", new String[]{id.toString()});
	...
	{  
        ...
        SQLiteDatabase db = DBHelper.getReadableDatabase();  
        Cursor cursor = db.rawQuery("select * from musiclist limit ?,?",  
